  ...
  return (
    <div className="rounded-xl border border-line bg-paper overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-line font-mono text-[11px] font-medium uppercase tracking-[0.04em] text-text3">
        ...
        Evidencia · computado sin IA · determinista
      </div>
      <div className="p-4 space-y-4">
        <p className="text-[11px] text-text3">Dato computado del instrumento, sin modelo de lenguaje. <b className="text-text2 font-medium">Tú interpretas.</b></p>
        {children}
        {fuente && <div className="font-mono text-[10px] text-text3 pt-2 border-t border-line/60">fuente: {fuente}</div>}
      </div>
    </div>
  );
}
...
export function EvidenceDivider() {
  return (
    <div className="flex items-center gap-2 font-mono text-[10px] text-text3 uppercase tracking-wide">
      ...
      ↓ interpretación · IA · valida antes de decidir
      <span className="h-px flex-1 bg-line" />
    </div>
  );
}
